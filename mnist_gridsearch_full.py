...
It implements the _full_ hyperparameter grid search that we would have run, given more time
"""
...
def main():
    ...
    n_train_samples = [5,10]#[1,2,5,10]
    KL_reweight = [False, True]
    gauss_prior_params = [0,1,2] # these are -log(sigma) values. make empty if u don't want gaussian priors
    mixture_prior_params = namedtuple('mixture_params', 'pis nl_sigma1s nl_sigma2s')(
                           [0.25, 0.5, 0.75],
                           [0,1,2],
                           [6,7,8]) # make empty if u don't want scale mixture priors
    ...
    for parameters in itertools.product([MLP, Dropout_MLP],
        ...
        run_experiment(args, parameters, outf, train_loader, valid_loader,
                       device, experiment_count)
        experiment_count += 1
        
    # Bayesian Net (only use vanilla Adam optimizer)
    for parameters in itertools.product([BayesianNet], units_per_layer, [(optim.Adam, {})]):
